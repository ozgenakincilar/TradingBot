...
- `BacktestExecutionSimulator`, decision replay'i bir sonraki signal candle açılışında sentetik spread, yönsel slippage, minimum latency, iki taraflı quote fee ve önceki kapalı candle hacmine dayalı katılım sınırıyla yürütür. Opsiyonel immutable `InstrumentRules` snapshot'ı alış fiyatını yukarı/satış fiyatını aşağı tick'e, miktarı lot adımına aşağı normalize eder ve minimum quantity/notional altı girişleri fill etmez.
