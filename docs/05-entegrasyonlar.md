...
2. Enstrüman kapısı geçerse `15m/200` kapalı signal geçmişi exact/contiguous doğrulanır.
3. Aynı `knownAt` ile `1H/200` kapalı trend geçmişi exact/contiguous doğrulanır.
4. İki candle serisi hazırsa WebSocket stream başlatılır ve buffer edilir.
5. REST başlangıç order-book snapshot'ı alınır.
6. Snapshot sequence ile buffer hizalanır.
7. İlk doğrulanmış event sonrası market-data ve genel readiness açılır.
8. Gap/stale/kopma durumunda ilgili sembol “not ready” yapılır.
9. REST snapshot ile onarım tamamlanınca yayın yeniden açılır.
...
`OkxSpotMarketStreamClient`, OKX public `books5` WebSocket snapshot kanalını `IMarketDataStreamClient` portuna dönüştürür. Payload'daki iki taraflı en fazla beş fiyat/miktar seviyesi bounded immutable depth olarak korunur; sırasız, crossed veya top-level ile çelişen veri fail-closed olur. REST recovery aynı sözleşme için `sz=5` ister. Subscribe acknowledgement market event sayılmaz; API error serbest metni sanitize edilir. Gerçek endpoint connectivity testi opt-in environment flag ile çalışır.
...
`OkxSpotInstrumentCatalog`, public instruments endpoint'ini `ISpotInstrumentCatalog` portuna dönüştürür. `OkxInstrumentStartupGate` hosted worker sıralamasında stream supervisor'dan önce çalışır; sembol/tür/filtre/state uyumsuzluğunda host başlangıcını durdurur. Aynı kapı, `15m/200` signal ve `1H/200` trend warm-up'larını ortak UTC bilgi anıyla sıralı tamamlar. Ortak `TradingReadinessState`, instrument, iki candle-history serisi ve ilk geçerli market event kapılarını ayrı izler; hepsi geçmeden OKX readiness açılmaz ve stream kesilince market-data readiness kapanır.
...
`ExportHistoricalCandleDataset`, uzun araştırma aralıklarını OKX'in resmî maksimum 100 candle sayfasına böler ve 20 istek/2 saniye endpoint sınırı için sayfa başlangıçlarını en az 100 ms aralıkla pace eder. `AtomicCsvHistoricalCandleDatasetSink` verified sayfaları canonical CSV partial dosyasına streaming yazar; tam SHA-256 sonrası atomik publish yapar, overwrite ve yarım artifact üretmez. Global çoklu-client rate limiter ayrıca tamamlanacaktır. [Resmi OKX TR candlestick history sözleşmesi](https://tr.okx.com/docs-v5/en/#order-book-trading-market-data-get-candlesticks-history)

`TradingBot.Research export-candles` yalnız public `https://www.okx.com/` endpoint'ine bağlanan, API key/secret parametresi olmayan operatör aracıdır. İlk stratejiyle uyumlu `BTC-USDT` benzeri Spot sembol, `15m|1H`, exact UTC aralık, source ID ve yeni `.csv` hedefini parse eder; artifact özetini JSON stdout'a, sanitize hatayı stderr'e yazar.
