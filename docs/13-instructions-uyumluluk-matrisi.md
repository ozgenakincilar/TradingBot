...
| 40 | Sell slippage/depth | 🟡 Kısmi | Strateji ve benchmark sell yönünde aynı önceki-kapalı-mum cost quote ve bounded TWAP çekirdeğini kullanır. OHLCV proxy yerine gerçek depth replay kanıtı kaldı. [TWAP modeli](../src/TradingBot.Application/Strategies/DynamicTwapExecutionModel.cs), [ADR-0028](adr/0028-dinamik-benchmark-twap-paritesi.md) |
...
| 44 | Düşük likidite | 🟡 Kısmi | Ortak strateji/benchmark çekirdeği toplam child miktarını bilinen mum hacminin en fazla `%5`iyle sınırlar; benchmark terminal pozisyonu kapasiteye sığmazsa reddeder. 24h volume/depth filtresi ve live parity kaldı. [Benchmark testleri](../tests/TradingBot.Application.Tests/BuyAndHoldBenchmarkTests.cs), [ADR-0028](adr/0028-dinamik-benchmark-twap-paritesi.md) |
| 45 | Gerçekçi fill süresi | 🟡 Kısmi | Strateji ve benchmark bounded child fill'leri latency sonrası deterministik TWAP zamanlarına yayar; girişte current-candle future range/volume sızmaz. Gerçek queue position, hidden liquidity ve cancel latency kaldı. [TWAP modeli](../src/TradingBot.Application/Strategies/DynamicTwapExecutionModel.cs), [ADR-0028](adr/0028-dinamik-benchmark-twap-paritesi.md) |
